...
one sig MobileMedia, Media, includeSorting, includeFavourites, includeCopyPhoto, Derivatives, includePrivacy, 
...
pred semanticsFM[] {
	isTrue[MobileMedia] and

    (isTrue[Media] <=> isTrue[MobileMedia]) and 
    (isTrue[Derivatives] <=> isTrue[MobileMedia]) and 

	(isTrue[includeSorting] => isTrue[MobileMedia]) and
	(isTrue[includeFavourites] => isTrue[MobileMedia]) and
	(isTrue[includeCopyPhoto] => isTrue[MobileMedia]) and
	(isTrue[includePrivacy] => isTrue[MobileMedia]) and
