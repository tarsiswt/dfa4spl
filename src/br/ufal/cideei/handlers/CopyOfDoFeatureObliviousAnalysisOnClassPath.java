...
import br.ufal.cideei.soot.analyses.wholeline.CopyOfWholeLineSimpleReachingDefinitionsAnalysis;
import br.ufal.cideei.soot.analyses.wholeline.CopyOfWholeLineSimpleUninitializedVariablesAnalysis;
...
public class CopyOfDoFeatureObliviousAnalysisOnClassPath extends AbstractHandler {
	...
	private void addPacks(IJavaProject javaProject, IClasspathEntry entry, String libs) {
		...
		Transform reachingDef = new Transform("jap.simplerd", CopyOfWholeLineSimpleReachingDefinitionsAnalysis.v());
		PackManager.v().getPack("jap").add(reachingDef);

		Transform uninitVars = new Transform("jap.simpleuv", CopyOfWholeLineSimpleUninitializedVariablesAnalysis.v());
		...
		double simpleRDTime = ((double) CopyOfWholeLineSimpleReachingDefinitionsAnalysis.v().getAnalysesTime()) / 1000000;
		double simpleUVTime = ((double) CopyOfWholeLineSimpleUninitializedVariablesAnalysis.v().getAnalysesTime()) / 1000000;

		CopyOfDoFeatureObliviousAnalysisOnClassPath.jimplificationResults.add(((double) (endJimplification - startJimplification)) / 1000000);
		CopyOfDoFeatureObliviousAnalysisOnClassPath.simpleRDResults.add(simpleRDTime);
		CopyOfDoFeatureObliviousAnalysisOnClassPath.simpleUVResults.add(simpleUVTime);
		...
		CopyOfWholeLineSimpleReachingDefinitionsAnalysis.v().reset();
		CopyOfWholeLineSimpleUninitializedVariablesAnalysis.v().reset();
