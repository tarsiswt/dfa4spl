...
public class LazyLiftedInitializedVariablesAnalysis extends ForwardFlowAnalysis<Unit, MapLiftedFlowSet> {
	...
	protected void merge(MapLiftedFlowSet source1, MapLiftedFlowSet source2, MapLiftedFlowSet dest) {
		source1.intersection(source2, dest);
	}
		...
		while (iterator.hasNext()) {
			...
			if (first.size() != 0) {
				if (first.size() == lazyConfig.size()) {
					gen(sourceFlowSet, unit, destFlowSet);
				} else {
					...
					gen(sourceFlowSet, unit, destToBeAppliedLattice);
	...
	private void gen(FlowSet source, Unit unit, FlowSet dest) {
